...
1. **检查文章格式**：确认 frontmatter 和内容符合规范
2. **获取并展示已有 tags**：查询当前博客中所有使用过的标签，展示给用户
   ```bash
   # 获取所有已使用的标签
   grep -rh "^  - " Blog/source/_posts/*/ 2>/dev/null | sort | uniq
   ```
3. **推荐 tags**：根据文章内容，为用户推荐合适的标签
4. **询问用户确认 tags 和 categories**：
   - 展示推荐的 tags 和已有标签列表
   - 让用户从推荐/已有标签中选择，或指定新的标签
   - 确认文章分类（`代码笔记` / `生活小记` / `读书笔记`）
5. **等待用户确认**：用户确认后，再设置到文章 frontmatter 中
6. **执行发布操作**
